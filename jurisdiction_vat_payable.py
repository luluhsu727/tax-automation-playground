...
def compute_vat_payable_by_jurisdiction(
    ...
    Each input item must include:
      - jurisdiction
      - taxable_amount
      - optional vat_rate (falls back to DEFAULT_VAT_RATES)
    """
...
def write_summary(path: Path, summary_rows: list[dict[str, str]]) -> None:
    """Write the jurisdiction-level VAT payable summary CSV."""
    fieldnames = (
        "jurisdiction",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the VAT to be paid for each jurisdiction.")
    parser.add_argument(
        "--input",
