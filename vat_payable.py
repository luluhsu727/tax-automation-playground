...
def transaction_from_record(record: dict[str, object]) -> Transaction:
    """Create a Transaction from an input dictionary.
    ...
    if tx_type not in {"sale", "purchase"}:
        raise ValueError("transaction_type/type must be either 'sale' or 'purchase'")
...
def load_transactions(path: str | Path) -> list[Transaction]:
    ...
    for record in records:
        if not isinstance(record, dict):
            raise ValueError("Each transaction record must be a JSON object")
...
def calculate_vat_payable_by_jurisdiction(
    ...
    *,
    ...
    for transaction in transactions:
        current = totals.get(transaction.jurisdiction, Decimal("0"))
