...
def _to_decimal(value: Any, field_name: str) -> Decimal:
    """Convert a value to Decimal with a clear validation message."""
...
def _normalize_rate(raw_rate: Any) -> Decimal:
    ...
    Supports decimal form (0.2 for 20%) or whole-percent form (20 for 20%).
    """
...
def _compute_transaction_vat(transaction: Mapping[str, Any]) -> Decimal:
    ...
    Precedence:
      1) explicit `vat_amount` if present
      2) derive from `net_amount * vat_rate`
    """
...
def generate_vat_summary_by_jurisdiction(
    ...
    """Generate output/input/payable VAT totals per jurisdiction."""
    ...
    for transaction in transactions:
        ...
        tx_type = _normalize_transaction_type(transaction.get("transaction_type"))
...
def generate_vat_payable_by_jurisdiction(
    ...
    """Return only VAT payable amount per jurisdiction."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate VAT payable amounts per jurisdiction from transactions CSV."
    )
    parser.add_argument("input_csv", help="Path to source transactions CSV")
    parser.add_argument(
        "-o",
        "--output",
