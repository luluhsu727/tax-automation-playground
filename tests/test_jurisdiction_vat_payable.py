...
class JurisdictionVatPayableTests(unittest.TestCase):
    ...
    def test_missing_default_rate_raises_error(self) -> None:
        ...
        with self.assertRaisesRegex(
            ValueError, "No VAT rate supplied for jurisdiction 'NL'"
        ):
            compute_vat_payable_by_jurisdiction(transactions)

    def test_supports_percent_form_vat_rate(self) -> None:
        transactions = [
            {"jurisdiction": "DE", "taxable_amount": "100.00", "vat_rate": "19"},
        ]

        summary = compute_vat_payable_by_jurisdiction(transactions)
        self.assertEqual(summary[0]["total_vat_payable"], "19.00")

    def test_calculate_vat_payable_with_sales_and_purchases(self) -> None:
        transactions = [
            {"jurisdiction": "DE", "type": "sale", "amount": "100.00", "vat_rate": "0.19"},
            {"jurisdiction": "DE", "type": "purchase", "amount": "20.00", "vat_rate": "0.19"},
            {"jurisdiction": "FR", "type": "sale", "amount": "50.00", "vat_rate": "0.20"},
        ]
