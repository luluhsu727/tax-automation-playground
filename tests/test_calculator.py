...
def test_calculates_payable_per_jurisdiction():
    transactions = [
...
def test_supports_explicit_vat_amount():
    transactions = [
        Transaction(
            jurisdiction="ES",
            amount=1000,
...
def test_supports_percentage_vat_rate():
    transactions = [Transaction(jurisdiction="IT", amount=100, vat_rate=22, transaction_type="sale")]
    result = calculate_vat_payable_by_jurisdiction(transactions)
    assert result["IT"]["output_vat"] == 22.0


def test_raises_for_invalid_transaction_type():
    transactions = [Transaction(jurisdiction="DE", amount=100, vat_rate=0.19, transaction_type="refund")]
