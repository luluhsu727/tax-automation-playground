...
class VatCalculatorTests(unittest.TestCase):
    def test_generates_vat_payable_by_jurisdiction_with_mixed_transactions(self):
        transactions = [
            {
                "jurisdiction": "DE",
    ...
    def test_refundable_jurisdiction_is_reported_when_input_exceeds_output(self):
        transactions = [
            {
                "jurisdiction": "ES",
                "transaction_type": "sale",
        ...
        self.assertEqual(result["ES"]["net_vat"], Decimal("-2.50"))
        self.assertEqual(result["ES"]["vat_payable"], Decimal("0.00"))
        self.assertEqual(result["ES"]["vat_refundable"], Decimal("2.50"))

    def test_missing_jurisdiction_raises_value_error(self):
