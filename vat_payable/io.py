...
def run_cli(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate VAT payable for each jurisdiction from a transactions CSV."
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    if args.format == "json":
        ...
        header = "jurisdiction,output_vat,input_vat,vat_payable"
        lines = [header]
        for row in rows:
            lines.append(
                f'{row["jurisdiction"]},{row["output_vat"]:.2f},{row["input_vat"]:.2f},{row["vat_payable"]:.2f}'
            )
        payload = "\n".join(lines)
