...
from vat_payable import (
    ...
    vat_payable_json_ready,
)


class VatPayableTests(unittest.TestCase):
    def test_calculate_mixed_sales_and_purchases(self) -> None:
        transactions = [
        ...
        self.assertEqual(actual, {"DE": Decimal("15.20"), "FR": Decimal("16.00")})

    def test_generate_vat_to_be_paid_for_each_jurisdiction(self) -> None:
        transactions = [
            {"jurisdiction": "es", "type": "sale", "amount": "100.00", "vat_rate": "0.21"},
            {"jurisdiction": "ES", "type": "purchase", "amount": "20.00", "vat_rate": "0.21"},
            {"jurisdiction": "IT", "type": "sale", "amount": "10.00", "vat_rate": "0.22"},
        ]

        result = generate_vat_to_be_paid_for_each_jurisdiction(transactions)

        self.assertEqual(result, {"ES": Decimal("16.80"), "IT": Decimal("2.20")})
