...
class VatPayableTests(unittest.TestCase):
    ...
    def test_defaults_transaction_type_to_sale(self) -> None:
        transactions = [
            {"jurisdiction": "ES", "amount": 100, "vat_rate": 0.21},
        ]
    ...
    def test_supports_country_field_as_alias(self) -> None:
        transactions = [
            {"country": "DE", "amount": 100, "vat_rate": 0.19},
        ]

        result = calculate_vat_payable_by_jurisdiction(transactions)

        self.assertEqual(result, {"DE": 19.0})

    def test_supports_net_amount_field(self) -> None:
        transactions = [
            {"jurisdiction": "IT", "net_amount": 100, "vat_rate": 22},
        ]
    ...
    def test_raises_for_missing_required_fields(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "missing amount/net_amount"):
            calculate_vat_payable_by_jurisdiction([{"jurisdiction": "DE", "vat_rate": 0.2}])

        with self.assertRaisesRegex(ValueError, "missing vat_rate"):
            calculate_vat_payable_by_jurisdiction([{"jurisdiction": "DE", "amount": 100}])
