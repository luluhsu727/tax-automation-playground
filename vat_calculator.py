...
VAT to be paid is computed as:
    output VAT (sales) - input VAT (purchases), floored at zero.
If input VAT exceeds output VAT, the difference is treated as a VAT credit.
"""
...
def _to_decimal(value: Any, *, field_name: str) -> Decimal:
    """Convert incoming numeric/string values to Decimal safely."""
    try:
...
def _money(amount: Decimal) -> Decimal:
    """Round to two decimal places using common financial rounding."""
...
class Transaction:
    ...
    def vat_amount(self) -> Decimal:
        """Compute VAT amount for this transaction."""
        if not self.taxable:
            return Decimal("0.00")
...
def calculate_vat_position_by_jurisdiction(
    ...
    """Calculate output/input VAT, payable, and credit for each jurisdiction."""
