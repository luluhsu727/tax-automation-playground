...
def process_transactions(
    ...
    with input_path.open("r", newline="", encoding="utf-8") as infile:
        ...
        for row in reader:
            ...
            revenue = _to_decimal(row["revenue"], "revenue")
            vat_collected = _to_decimal(row["vat_collected"], "vat_collected")
            expected_vat = _quantize(revenue * vat_rate)
            vat_payable = _quantize(expected_vat - vat_collected)

            row.update(
                {
                    jurisdiction_column: jurisdiction,
                    "revenue": _format_decimal(revenue),
                    "vat_collected": _format_decimal(vat_collected),
                    "vat_rate": _format_decimal(vat_rate),
                    "expected_vat": _format_decimal(expected_vat),
                    "vat_payable": _format_decimal(vat_payable),
                }
            )
            ...
            writer.writerow(
                {
                    ...
                    "total_vat_collected": _format_decimal(
                        totals["total_vat_collected"]
                    ),
                    "total_expected_vat": _format_decimal(totals["total_expected_vat"]),
                    "vat_payable": _format_decimal(vat_payable),
                }
            )
