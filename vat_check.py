...
def process_transactions(
    ...
    with input_path.open("r", newline="", encoding="utf-8") as infile:
        ...
        for row in reader:
            ...
            revenue = _to_money(_parse_decimal(row["revenue"], "revenue"))
            vat_collected = _to_money(_parse_decimal(row["vat_collected"], "vat_collected"))
    ...
    with summary_output_path.open("w", newline="", encoding="utf-8") as summary_file:
        ...
        for jurisdiction in sorted(summary_totals):
            ...
            total_revenue = _to_money(totals["total_revenue"])
            total_vat_collected = _to_money(totals["total_vat_collected"])
            total_expected_vat = _to_money(totals["total_expected_vat"])
            vat_payable = _to_money(total_expected_vat - total_vat_collected)
            writer.writerow(
                {
                    "jurisdiction": jurisdiction,
                    "total_revenue": f"{total_revenue:.2f}",
                    "total_vat_collected": f"{total_vat_collected:.2f}",
                    "total_expected_vat": f"{total_expected_vat:.2f}",
                    "vat_payable": f"{vat_payable:.2f}",
                }
            )
